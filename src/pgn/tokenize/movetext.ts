...
import type { PgnMovetextToken, TokenReturn } from "./types.js";
...
export function* handleMovetext(reader: AbstractReader): TokenReturn<PgnMovetextToken> {
    ...
    if (reader.match(T.LEFT_PARENTHESIS)){
        ...
        while (!reader.isAtEnd() && reader.get() != T.RIGHT_PARENTHESIS){
            ...
            if (isWhitespace(v)){
                ...
                movetextTokens.push(yield* handleComment(reader));
            }else if (T.SAN_GLYPHS.has(v)){
                movetextTokens.push(yield* handleSanGlyph(reader));
            }else if (v == T.DOLLAR_SIGN){
                movetextTokens.push(yield* handleNag(reader));
            }else{
                movetextTokens.push(yield* handleMovetext(reader));
            }
        }
    ...
    if (isNumber(reader.get())){
        const firstNum = yield* handleNumber(reader);
        ...
        if (reader.match(T.DOT)){
            ...
            reader.copyReject();
            reader.skipWhitespace();
            const secondNum = yield* handleNumber(reader);
            return {
                type: "result",
